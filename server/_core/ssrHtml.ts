...
const CANONICAL_ORIGIN = (
  process.env.CANONICAL_ORIGIN || "https://akbarnawasunda.my.id"
).replace(/\/$/, "");
...
const metaText = (value: string, max: number) =>
  clampText(value.replace(/[#*_`~]+/g, ""), max);
...
export function buildHeadTags(head: HeadMeta): string {
  ...
  const canonicalUrl = head.canonicalPath
    ? `${CANONICAL_ORIGIN}${head.canonicalPath}`
    : "";
  ...
  if (canonicalUrl) {
    ...
    if (!head.noindex && !head.notFound) {
      ...
      const isEnglish =
        canonicalPath === "/en" || canonicalPath.startsWith("/en/");
      const idPath = isEnglish
...
export function composeHtml(
  ...
  const structuredData = head.structuredData
    ? structuredDataScript(head.structuredData)
    : "";
