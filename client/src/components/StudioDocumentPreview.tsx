import { ArrowUpRight, Eye, ExternalLink, FileText, Image as ImageIcon, Link2 } from "lucide-react";
import { StudioAssetPreview, StudioLinkPreview } from "./StudioAssetPreview";

type DocumentType = "hero" | "profile" | "pressKit" | "siteSettings" | "legal" | "release" | "visual" | "live" | "event";
...
function linkValues(payload: Payload) {
  const keys = ["primaryActionUrl", "url", "embedUrl", "spotifyUrl", "appleMusicUrl", "locationUrl", "mapsUrl", "ticketUrl", "rsvpUrl", "actionUrl", "canonicalUrl"];
  const links = keys.map(key => ({ label: key, href: value(payload, key) })).filter(link => link.href);
  ...
  return links.filter((link, index, all) => all.findIndex(item => item.href === link.href) === index).slice(0, 8);
}

function textValue(payload: Payload) {
  return ["heroBody", "shortBio", "intro", "story", "message", "artistStatement", "metaDescription", "ogDescription", "description", "body"]
    .map(key => value(payload, key))
  ...
  return (
    <section className="mt-5 overflow-hidden rounded-2xl border border-violet-200/15 bg-violet-200/[0.035]" aria-label="Live document preview">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/[0.08] px-5 py-4 sm:px-6">
        <div><div className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.2em] text-violet-200/75"><Eye size={13} />03 // Live preview</div><h3 className="mt-2 text-lg font-semibold text-white">Yang akan terlihat di publik</h3><p className="mt-1 text-xs text-white/40">{route.surface} · {route.route} · {slug || "default"}</p></div>
        <a href={route.route} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-violet-200/20 bg-violet-200/[0.06] px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.1em] text-violet-100/80 transition hover:bg-violet-200/[0.12]">Buka halaman <ExternalLink size={12} /></a>
      </div>
      <div className="grid gap-5 p-5 sm:p-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <div className="min-w-0">
          <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/15 px-2.5 py-1 font-mono text-[9px] uppercase tracking-[0.14em] text-white/45"><FileText size={12} />{documentType}</span>
          <h4 className="mt-4 text-2xl font-semibold tracking-tight text-white">{title}</h4>
          <p className="mt-3 text-xs leading-5 text-white/50">{textValue(payload)}</p>
      ...
      <div className="flex items-center gap-2 border-t border-white/[0.08] px-5 py-3 text-[10px] leading-4 text-white/35 sm:px-6"><ArrowUpRight size={12} className="shrink-0 text-violet-200/60" />Preview ini mengikuti draft saat ini; perubahan baru tampil ke publik setelah disimpan sebagai Published.</div>
    </section>
  );
}
