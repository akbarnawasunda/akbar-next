...
export function BrandMotionMark({ src }: { src: string }) {
  ...
  useEffect(() => {
    ...
    for (let y = 0; y < sample.height; y += step * pixelRatio) {
      for (let x = 0; x < sample.width; x += step * pixelRatio) {
        ...
        const introRadius = Math.max(width, height) * (0.18 + Math.random() * 0.28);
        const dissolveRadius = Math.max(width, height) * (0.58 + Math.random() * 0.46);
        const blueParticle = isBlueAccent && blue > green + 8;
        const color = blueParticle ? "#78a8df" : Math.random() > 0.9 ? "#c7794c" : "#ece6dc";
        ...
        candidates.push({
          ...
          color,
          size: (mobile ? 1 : 1.15) + (Math.random() > 0.92 ? 0.85 : Math.random() * 0.35),
    ...
    const duration = initialFormation ? (mobile ? 1180 : 1420) : (mobile ? 1320 : 1540);
    let startedAt = 0;
    ...
    const draw = (time: number) => {
      ...
      particles.forEach(particle => {
        ...
        if (initialFormation) {
          ...
          x = (particle.tx + particle.dx) + ((particle.tx - (particle.tx + particle.dx)) * reform);
          y = (particle.ty + particle.dy) + ((particle.ty - (particle.ty + particle.dy)) * reform);
  ...
  const replay = () => {
    if (!runningRef.current && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
  ...
  return (
    ...
    >
      <img ref={imageRef} className="an-rmx-particle-source" crossOrigin="anonymous" src={src} alt="" onLoad={() => setAssetReady(true)} />
      <canvas ref={canvasRef} aria-hidden="true" />
      <span className="an-rmx-particle-access">Ketuk untuk memecah dan membentuk ulang logo partikel.</span>
    </button>
  );
}
