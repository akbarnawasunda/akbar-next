...
export function OfficialMediaFrame({ title, provider, sourceUrl, embedUrl, artwork, description }: OfficialMediaFrameProps) {
  ...
  return <article className={`an-official-media${playerRequested ? " is-player-open" : ""}`}>
    <a className="an-official-media-art" href={sourceUrl} target="_blank" rel="noreferrer">
      <img src={artwork} alt={`Artwork resmi untuk ${title}`} onError={event => { event.currentTarget.src = "/favicon.ico"; }} />
    ...
    <div className="an-official-media-actions"><a href={sourceUrl} target="_blank" rel="noreferrer">OPEN {provider.toUpperCase()} <ArrowUpRight size={14} /></a><button type="button" onClick={openPlayer}>{playerRequested ? "RELOAD PLAYER" : "PLAY HERE"} <Play size={13} fill="currentColor" /></button></div>
    {playerRequested && <div className={`an-official-player${playerLoaded ? " is-loaded" : ""}`}><iframe title={`${provider} player: ${title}`} src={embedUrl} allow="autoplay; encrypted-media; picture-in-picture; web-share" allowFullScreen onLoad={() => setPlayerLoaded(true)} />{!playerLoaded && <span>CONNECTING TO {provider.toUpperCase()}…</span>}{slowPlayer && <p>Player belum merespons di browser ini. Gunakan tombol <strong>OPEN {provider.toUpperCase()}</strong> di atas untuk membuka sumber resminya.</p>}</div>}
  </article>;
}
