...
export function EnglishHome() {
  ...
  return <EnglishFrame><main id="top" className="en-content en-home-parity">
    ...
    <section className="section live-section reveal-target"><SectionIndex number="05" label="LIVE SIGNAL" /><div className="live-backdrop" style={{ backgroundImage: "url(/assets/akbar-night-frequency-hero.webp)" }} /><div className="live-copy"><p className="eyebrow">LIVE</p><h2>LIVE<br />DATES.</h2><p>{cms.data?.live?.message || "Dates will appear here after they are officially announced."}</p><Link className="button-primary" href={featuredEvent?.ticketUrl || featuredEvent?.rsvpUrl || "/en/inquire"} target={featuredEvent?.ticketUrl || featuredEvent?.rsvpUrl ? "_blank" : undefined} rel={featuredEvent?.ticketUrl || featuredEvent?.rsvpUrl ? "noreferrer" : undefined}><Ticket size={16} /><span>{featuredEvent?.ticketUrl ? "GET TICKETS" : featuredEvent?.rsvpUrl ? "RSVP SHOW" : "ASK ABOUT BOOKING"}</span></Link></div><div className="live-status"><span>{featuredEvent ? "NEXT CONFIRMED SHOW" : "LIVE SIGNAL"}</span><strong>{featuredEvent?.title || <>NO PUBLIC<br />DATE</>}</strong><small>{featuredEvent ? formatEnglishDate(featuredEvent.date) : "TO BE ANNOUNCED"}</small></div>{cms.data?.events.length ? <div className="home-live-events" aria-label="Confirmed live dates">{cms.data.events.slice(0, 3).map(event => { const eventHref = event.ticketUrl || event.rsvpUrl || "/en/live"; return <a className="home-live-event" href={eventHref} target={eventHref.startsWith("http") ? "_blank" : undefined} rel={eventHref.startsWith("http") ? "noreferrer" : undefined} key={event._id}><span>{formatEnglishDate(event.date, event.time)}</span><strong>{event.title}</strong><small>{[event.venue, event.city, event.country].filter(Boolean).join(", ") || "Venue details to follow"}</small><ArrowUpRight size={14} /></a>; })}</div> : null}</section>
...
export function EnglishMusic() {
  ...
  return <EnglishFrame><main className="en-content"><section className="nf-page-hero en-hero" style={{ "--page-image": `url(${featured.image || officialBrand.socialPreview})` } as React.CSSProperties}><div><p className="nf-page-eyebrow">MUSIC / OFFICIAL CATALOG</p><h1>MUSIC<br /><em>ARCHIVE.</em></h1><p>Original releases and official remixes by Akbar Nawasunda.</p></div><div className="nf-hero-note"><span>{cms.isLoading ? "LOADING CATALOG" : "FEATURED RELEASE"}</span><strong>{featured.title}</strong><a className="nf-text-button" href={featured.href} target="_blank" rel="noreferrer">LISTEN <ArrowUpRight size={15} /></a></div></section>
    <section className="nf-section"><div className="en-section-intro"><p className="nf-page-eyebrow">OFFICIAL PLATFORM ROUTES</p><h2>CHOOSE<br /><em>YOUR PLATFORM.</em></h2><p>Use the official links below. Player embeds remain optional and load only when requested.</p></div><div className="nf-platform-grid">{editablePlatformLinks.map(platform => <a key={platform.label} href={platform.href} target="_blank" rel="noreferrer"><PlatformIcon label={platform.label} />{platform.label}<ArrowUpRight size={15} /></a>)}</div></section>
