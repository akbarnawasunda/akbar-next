...
export function EnglishHome() {
  ...
  return <EnglishFrame><main id="top" className="en-content en-home-parity">
    <section className="an-hero en-parity-hero">
      ...
      <div className="hero-copy"><p className="eyebrow"><span /> AKBAR NAWASUNDA</p><h1 className="hero-title-editorial" data-no-scramble="true" aria-label="Akbar Nawasunda"><span aria-hidden="true"><span className="hero-title-mask"><span className="hero-title-word">AKBAR</span></span>{" "}<span className="hero-title-mask"><span className="hero-title-word">NAWASUNDA.</span></span></span></h1><p className="hero-description">Producer, remixer, and electronic bass artist from Bandung Barat, Indonesia.</p><div className="hero-actions"><a className="button-primary" href={activeRelease.href} target="_blank" rel="noreferrer"><Play size={15} fill="currentColor" /><span>LISTEN NOW</span></a><Link className="button-quiet" href="/en/visuals">VIEW VISUALS <ArrowUpRight size={16} /></Link><a className="hero-signal-link" href="#signal">FAN SIGNAL <ArrowDownRight size={14} /></a></div></div>
    ...
    <section className="home-signal-deck reveal-target" aria-labelledby="en-signal-deck-title"><SectionIndex number="01" label="OFFICIAL NETWORK" /><div className="home-signal-copy"><p className="eyebrow"><span /> OFFICIAL PLATFORMS</p><h2 id="en-signal-deck-title">LISTEN<br />WHERE YOU WANT.</h2><Link className="home-deck-cta" href="/en/music">VIEW MUSIC <ArrowUpRight size={15} /></Link></div><div className="home-platform-rack">{allPlatformLinks.map((platform, index) => <a className={`home-platform-card platform-${platform.label.toLowerCase().replace(/\s+/g, "-")}`} key={platform.label} href={platform.href} target="_blank" rel="noreferrer" aria-label={`Open Akbar Nawasunda on ${platform.label}`}><span className="home-platform-number">0{index + 1}</span><span className="home-platform-icon-shell"><PlatformIcon label={platform.label} /></span><span className="home-platform-copy"><strong>{platform.label}</strong><small>OFFICIAL PLATFORM</small></span><ArrowUpRight className="home-platform-arrow" size={15} /></a>)}</div><PlatformMarquee /></section>

    <section className="section section-current reveal-target" id="music"><SectionIndex number="02" label="CURRENT RELEASE" /><div className="section-heading"><p className="eyebrow">LATEST RELEASE</p><h2>NOW<br />PLAYING.</h2></div><div className="feature-release"><div className="release-cover"><ResilientArtworkImage src={activeRelease.image} backupSrc={officialBrand.socialPreview} alt={`Artwork for ${activeRelease.title}`} /><span className="cover-orbit" /></div><div className="release-detail"><p className="mono-label">{activeRelease.format} · {activeRelease.year}</p><h3>{activeRelease.title}</h3><p>Open the official release route to listen to this verified catalog entry.</p><div className="release-detail-actions"><a className="text-link" href={activeRelease.href} target="_blank" rel="noreferrer">OPEN RELEASE <ArrowUpRight size={16} /></a><span>{activeRelease.platform}</span></div></div></div></section>

    <section className="section release-section reveal-target"><SectionIndex number="03" label="DISCOGRAPHY" /><div className="section-inline"><div><p className="eyebrow">DISCOGRAPHY</p><h2>ALL<br />RELEASES.</h2></div><a className="text-link" href="https://open.spotify.com/intl-id/artist/7KOQuIQLuxyklLox0RDMMw" target="_blank" rel="noreferrer">SPOTIFY <PlatformIcon label="Spotify" /> <ArrowUpRight size={16} /></a></div><div className="release-grid">{catalog.map((release, index) => <a className="release-card" key={release.title} href={release.href} target="_blank" rel="noreferrer">{release.image && <div className="release-card-art"><ResilientArtworkImage src={release.image} backupSrc={officialBrand.socialPreview} alt={`Artwork for ${release.title}`} /></div>}<span className="release-number">0{index + 1}</span><PlatformIcon label={release.platform} /><p>{release.format} · {release.year}</p><h3>{release.title}</h3><span className="release-platform-line">{release.platform} <ArrowUpRight size={14} /></span></a>)}</div></section>

    <section className="section visual-section reveal-target"><SectionIndex number="04" label="OFFICIAL VISUALS" /><div className="section-heading"><p className="eyebrow">VISUAL</p><h2>OFFICIAL<br />VIDEO.</h2></div><div className="video-grid">{activeVideos.map((video, index) => <a key={video.title} className={`video-card video-${index + 1}`} href={video.href} target="_blank" rel="noreferrer"><ResilientArtworkImage src={video.image} backupSrc={officialBrand.socialPreview} alt={`${video.title} — official visual artwork`} /><div className="video-overlay" /><div className="video-content"><span>{video.label}</span><h3>{video.title}</h3><div className="round-play"><Play size={17} fill="currentColor" /></div></div></a>)}</div></section>

    <section className="section live-section reveal-target"><SectionIndex number="05" label="LIVE SIGNAL" /><div className="live-backdrop" style={{ backgroundImage: "url(/assets/akbar-night-frequency-hero.webp)" }} /><div className="live-copy"><p className="eyebrow">LIVE</p><h2>LIVE<br />DATES.</h2><p>Dates will appear here after they are officially announced.</p><Link className="button-primary" href="/en/inquire"><Ticket size={16} /><span>ASK ABOUT BOOKING</span></Link></div><div className="live-status"><span>{featuredEvent ? "NEXT CONFIRMED SHOW" : "LIVE SIGNAL"}</span><strong>{featuredEvent?.title || <>NO PUBLIC<br />DATE</>}</strong><small>{featuredEvent ? "OFFICIAL DATE" : "TO BE ANNOUNCED"}</small></div></section>

    <section className="signal-section reveal-target" id="signal"><SectionIndex number="06" label="FAN SIGNAL" /><div><p className="eyebrow"><Sparkles size={14} /> FAN SIGNAL</p><h2>RELEASE<br />UPDATES.</h2><p>Official release, visual, and live updates from Akbar Nawasunda's channels.</p></div><div className="en-signal-action"><strong>STAY CLOSE TO THE SOURCE.</strong><p>Follow the official platforms for new music and future announcements.</p><Link className="button-primary" href="/en/inquire">OPEN CONTACT ROUTE <ArrowUpRight size={15} /></Link></div></section>
  </main></EnglishFrame>;
}
...
export function EnglishEpk() {
  ...
  return <EnglishFrame><main className="en-content">
    ...
    <section className="nf-section"><div className="en-section-intro"><p className="nf-page-eyebrow">OFFICIAL CHANNELS</p><h2>OFFICIAL<br /><em>PLATFORMS.</em></h2></div><div className="nf-platform-grid">{allPlatformLinks.slice(0, 5).map(link => <a href={link.href} target="_blank" rel="noreferrer" key={link.label}><PlatformIcon label={link.label} />{link.label}<ArrowUpRight size={15} /></a>)}</div></section>
  </main></EnglishFrame>;
}
