...
export function EnglishVisuals() {
  ...
  return <EnglishFrame><main className="en-content"><section className="nf-page-hero en-hero" style={{ "--page-image": `url(${portraitStudies[0].src})` } as React.CSSProperties}><div><p className="nf-page-eyebrow">VISUALS / OFFICIAL CHANNEL</p><h1>VIDEO<br /><em>ARCHIVE.</em></h1><p>Visual releases, DJ edits, and official uploads from the Akbar Nawasunda channel.</p></div><div className="nf-hero-note"><span>{cms.isLoading ? "LOADING VISUALS" : "OFFICIAL CHANNEL"}</span><strong>AKBAR NAWASUNDA</strong><a className="nf-text-button" href="https://www.youtube.com/@akbarnawasunda" target="_blank" rel="noreferrer">OPEN YOUTUBE <ArrowUpRight size={15} /></a></div></section><section className="nf-section"><div className="en-section-intro"><p className="nf-page-eyebrow">SELECTED VISUALS</p><h2>WATCH<br /><em>THE VIDEOS.</em></h2><p>Preview thumbnails first. Embedded players load only after you choose to play.</p></div><div className="en-visual-grid">{visualItems.map(item => item.youtubeId ? <div className="en-visual-card" key={item.title}><OfficialMediaFrame title={item.title} provider="YouTube" sourceUrl={item.href} embedUrl={`https://www.youtube-nocookie.com/embed/${item.youtubeId}`} artwork={item.image} backupArtwork={officialBrand.socialPreview} description="Official video channel." /></div> : <a className="en-service-card" href={item.href} target="_blank" rel="noreferrer"><span>{item.label}</span><h3>{item.title}</h3><p>Open the official source to view this visual.</p><ArrowUpRight size={17} /></a>)}</div></section><VisualPortraitStudies english /> <section className="nf-section dark-panel"><div className="en-contact-panel"><div><p className="nf-page-eyebrow">DIRECTOR / BOOKER / CURATOR</p><h2>NEED THE<br /><em>RIGHT CUT?</em></h2><p>For visual collaboration, licensing, or a release-related brief, send the context directly.</p></div><a className="nf-button" href={`mailto:${bookingEmail}?subject=Visual%20inquiry`}><Mail size={15} /> EMAIL THE STUDIO <ArrowUpRight size={15} /></a></div></section></main></EnglishFrame>;
}
