...
export function EnglishMusic() {
  ...
  const featured: CatalogItem = current
    ? catalog.find(
        (item) =>
          item.title.trim().toLowerCase() ===
    ...
    <EnglishFrame>
      <main className="en-content">
        <section
          ...
          style={
            {
              "--page-image": `url(${featured.image || officialBrand.socialPreview})`,
            } as CSSProperties
          }
        >
          <div>
            ...
            <p>Original releases and remixes by Akbar Nawasunda.</p>
          </div>
          <div className="nf-hero-note">
            <span>
              {cms.isLoading ? "LOADING CATALOG" : "FEATURED RELEASE"}
            </span>
        ...
        <section className="nf-section">
          <div className="en-section-intro">
            ...
            <p>
              Use the official links below. Player embeds remain optional and
          ...
          <div className="nf-platform-grid">
            {editablePlatformLinks.map((platform) => (
              ...
              >
                <PlatformIcon label={platform.label} />
                <span className="nf-platform-label">{platform.label}</span>
        ...
        <section className="nf-section">
          <div className="en-two-column">
            <div className="en-aside">
              <p className="nf-page-eyebrow">FEATURED RELEASE</p>
              <strong>{featured.title}</strong>
              <span>
                {featured.format || "Release"} · {featured.year || "2025"} ·{" "}
                {featured.platform || "Official link"}
              </span>
            </div>
            <div className="en-copy-block">
              ...
              <p>
                Release details will appear here when published. A listening
                source is always available.
              </p>
              {featured.credits && (
                <p className="en-inline-note">CREDITS · {featured.credits}</p>
              )}
              <Link
                className="nf-text-button"
                href={`/en/music/${releaseSlug(featured.title)}`}
              >
                OPEN RELEASE DETAIL <ArrowUpRight size={14} />
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="nf-section dark-panel">
          <div className="en-section-intro">
            ...
            <p>
              Open a player when you want it, or use the official source link on
              every card.
            </p>
          </div>
          <div className="en-visual-grid">
            {players.map((drop) => {
              ...
              return (
                <div className="en-visual-card" key={drop.url}>
                  <OfficialMediaFrame
                    title={drop.title}
                    provider="SoundCloud"
                    sourceUrl={drop.url}
          ...
          <div className="en-catalog">
            {catalog.map((item, index) => (
              <Link
                ...
                className="en-release-card"
                href={`/en/music/${releaseSlug(item.title)}`}
              >
                <ResilientArtworkImage
                  src={item.image}
                  backupSrc={officialBrand.socialPreview}
                ...
                />
                <div className="en-release-card-copy">
                  <span>
                    {formatPublicIndex(index)} / {item.format} · {item.year}
                  </span>
                  <h3>{item.title}</h3>
                  <p>{item.platform}</p>
                </div>
...
export function EnglishVisuals() {
  ...
  const visualItems = cmsVisuals.length
    ? cmsVisuals.map((item) => ({
        title: item.title,
        label: item.label || "OFFICIAL VISUAL",
        href:
          item.url ||
          (item.youtubeId
    ...
    <EnglishFrame>
      <main className="en-content">
        <section
          ...
          style={
            {
              "--page-image": `url(${portraitContent[0]?.imageUrl || officialBrand.portrait})`,
            } as CSSProperties
          }
        >
          <div>
            ...
            <p>
              Visual releases, DJ edits, and official uploads from the Akbar
              Nawasunda channel.
            </p>
          </div>
          <div className="nf-hero-note">
            <span>
              {cms.isLoading ? "LOADING VISUALS" : "OFFICIAL CHANNEL"}
            </span>
        ...
        <section className="nf-section">
          <div className="en-section-intro">
            ...
            <p>
              Preview thumbnails first. Embedded players load only after you
              choose to play.
            </p>
          </div>
          <div className="en-visual-grid">
            {visualItems.map((item) =>
              item.youtubeId ? (
                <div className="en-visual-card" key={item.title}>
                  <OfficialMediaFrame
                    title={item.title}
                    provider="YouTube"
                    sourceUrl={item.href}
                ...
                >
                  ...
                  <h3>{item.title}</h3>
                  <p>Open the official source to view this visual.</p>
        ...
        <VisualPortraitStudies english studies={portraitContent} />
        ...
        <section className="nf-section dark-panel">
          <div className="en-contact-panel">
            <div>
              ...
              <p>
                For visual collaboration, licensing, or a release-related brief,
                send the context directly.
              </p>
            </div>
            <a
              className="nf-button"
              href={`mailto:${bookingEmail}?subject=Visual%20inquiry`}
            >
              <Mail size={15} /> EMAIL THE STUDIO <ArrowUpRight size={14} />
            </a>
          </div>
        </section>
      </main>
      ...
      <main className="en-content">
        <section className="nf-page-hero en-hero">
          <div>
            ...
            <p>
              {cms.data?.live?.message ||
                "Public dates will appear here after they are officially announced."}
            </p>
          </div>
          <div className="nf-hero-note">
            <span>
              {featured ? "NEXT CONFIRMED SHOW" : "CURRENT STATUS"}
            </span>
            <strong>{featured?.title || "NO PUBLIC DATE"}</strong>
            {featured?.ticketUrl ? (
              <a
                className="nf-text-button"
              ...
              <Link className="nf-text-button" href="/en/inquire">
                BOOKING INQUIRY <ArrowUpRight size={14} />
              </Link>
            )}
          </div>
        </section>
        ...
        <section className="nf-section">
          <div className="en-section-intro">
            ...
            <p>
              Only confirmed, publicly available dates are shown. No placeholder
          ...
          {events.length ? (
            <div className="en-service-grid">
              {events.map((event) => (
                <article className="en-service-card" key={event._id}>
                  <span>
                    <CalendarDays size={13} />{" "}
                    {formatEnglishDate(event.date, event.time)}
                  </span>
                  <h3>{event.title}</h3>
                  <p>
                    {event.mapsUrl ? (
            ...
            <div className="en-empty">
              <span className="nf-page-eyebrow">DATES / STANDBY</span>
              <strong>No confirmed show is public yet.</strong>
              <p>
                The calendar stays quiet until a real date, place, and official
                route are ready to share.
              </p>
              <Link className="nf-button" href="/en/inquire">
                ASK ABOUT BOOKING <ArrowUpRight size={14} />
              </Link>
            </div>
          )}
        </section>
        ...
        <section className="nf-section dark-panel">
          <div className="en-two-column">
            <div className="en-aside">
              ...
              <strong>Keep the line open.</strong>
              <p>
                Follow the official platforms for new releases and future live
                announcements.
              </p>
            </div>
            <div className="en-copy-block">
              ...
              <p>
                Share your city, date window, venue context, and project brief.
                Availability and terms are discussed directly.
              </p>
              <a
                className="nf-button"
                href={`mailto:${bookingEmail}?subject=Live%20booking%20inquiry`}
              >
                EMAIL BOOKING <ArrowUpRight size={14} />
              </a>
            </div>
          </div>
        </section>
      ...
      <main className="en-content">
        <section className="nf-page-hero en-hero">
          <div>
            ...
            <p>
              A concise route through the sound, identity, and catalog behind
              Akbar Nawasunda.
            </p>
          </div>
          <div className="nf-hero-note">
        ...
        <section className="nf-section" id="origin">
          <div className="en-two-column">
            ...
            <div className="en-copy-block">
              ...
              <p>
                {journey.introEn ||
                  "Akbar Nawasunda began creating independently in 2020 as DJ Akbar Remix, experimenting with Breakbeat, Jedag Jedug, and Jungle Dutch through a Bandung-rooted remix language. The current name carries that energy into original releases that connect pop melody, electronic bass, and remix instinct."}
              </p>
              <div className="en-stat-row">
                <div className="en-stat">
                  <strong>2020</strong>
                  <span>CREATIVE ORIGIN</span>
                </div>
                <div className="en-stat">
                  <strong>{verifiedArtistProfile.location.split(",")[0]}</strong>
                  <span>HOME BASE</span>
                </div>
              </div>
              <div className="en-genre-row">
        ...
        <section className="nf-section en-archive-art-feature">
          <div className="en-archive-art-frame">
            <img
              src={officialBrand.archivePortrait}
              alt="Editorial Akbar Nawasunda artwork about a future city"
              loading="lazy"
              decoding="async"
              width={800}
              height={1000}
          ...
          <div className="en-archive-art-copy">
            ...
            <p>
              One visual direction from the Akbar Nawasunda world: industrial,
              high-contrast, and close to electronic bass energy.
            </p>
            <p className="en-archive-art-note">
              ARCHIVE VISUAL · NOT AN AUDIO RELEASE
            </p>
            <Link className="nf-button" href="/en/visuals">
              VIEW VISUALS <ArrowUpRight size={14} />
            </Link>
          </div>
        </section>
    ...
    <EnglishFrame>
      <main className="en-content">
        <section
          ...
          style={
            {
              "--page-image": `url(${profile?.portraitImage || officialBrand.portrait})`,
            } as CSSProperties
          }
        >
          <div>
          ...
          <div className="nf-hero-note">
            ...
            <Link className="nf-text-button" href="/en/music">
              VIEW MUSIC <ArrowUpRight size={14} />
            </Link>
          </div>
        </section>
        ...
        <section className="nf-section an-profile-section">
          <div className="an-profile-aside">
            <p className="nf-page-eyebrow">ARTIST PROFILE</p>
            <MapPin size={17} />
            {locationUrl ? (
              <a
            ...
            <span>
              ALSO KNOWN AS {verifiedArtistProfile.aliases.join(" / ")}
            </span>
          </div>
          <div className="an-profile-copy">
            ...
            {profile?.artistStatement ? (
              ...
              <p className="an-profile-note">
                Original work is released as Akbar Nawasunda; the remix catalog
                is also known through DJ Akbar Remix.
              </p>
            )}
        ...
        <section className="nf-section dark-panel an-about-path">
          ...
          <div className="an-about-actions">
            <Link className="nf-button" href="/en/music">
              <Sparkles size={15} /> MUSIC
            </Link>
            <Link className="nf-text-button" href="/en/epk">
              PRESS & BOOKING <ArrowUpRight size={14} />
            </Link>
          </div>
        </section>
      </main>
  ...
  const optionalAssets = [
    {
      ...
      url: pressKit?.oneSheetUrl,
    },
    {
      label: "PRESS IMAGES",
      title: "Press image set",
      url: pressKit?.photoPackUrl,
    },
    {
      label: "BRAND KIT",
      title: "Logo package",
      url: pressKit?.logoPackUrl,
    },
    {
      label: "SHOW NOTES",
      title: "Technical rider",
      url: pressKit?.technicalRiderUrl,
    },
      ...
      <main className="en-content">
        <section className="nf-epk-hero en-hero en-epk-hero">
          <div className="en-epk-hero-copy">
            ...
            <p>
              Information for promoters, media, playlist editors, and
              collaborators.
            </p>
            <div className="en-action-row">
              <a
                className="nf-button"
                href={`mailto:${email}?subject=Akbar%20Nawasunda%20press%20or%20booking`}
              >
                <Mail size={14} /> CONTACT PRESS
              </a>
              <Link className="nf-text-button" href="/en/about">
                READ ARTIST PROFILE <ArrowUpRight size={14} />
              </Link>
            </div>
          </div>
          <aside className="en-epk-art-card">
            <img
            ...
            <div>
              ...
              <small>
                ...
                {verifiedArtistProfile.aliases[0]}
              </small>
            </div>
          </aside>
        </section>
        ...
        <section className="nf-section">
          <div className="en-two-column">
            ...
            <aside className="en-aside">
              <p className="nf-page-eyebrow">QUICK FACTS</p>
              <strong>{verifiedArtistProfile.location}</strong>
              <span>
                Also known as {verifiedArtistProfile.aliases.join(" / ")}
              </span>
              <span>Producer / Remixer</span>
              <span>{email}</span>
            </aside>
          </div>
        </section>
        ...
        <section className="nf-section dark-panel">
          <div className="en-section-intro">
            ...
            <p>
              Working formats available for performance, production,
              collaboration, and music use.
            </p>
          </div>
          <div className="en-service-grid">
            {verifiedArtistProfile.services.map((service, index) => (
              <article className="en-service-card" key={service}>
                <span>{formatPublicIndex(index)}</span>
                <h3>{service}</h3>
                <p>
                  {index === 0
                    ? "Remix requests with a clear brief, references, and release direction."
                    : index === 1
                      ? "Custom arrangements shaped around the character of a track or content brief."
                      : index === 2
                        ? "Build a focused piece together from first idea through publication-ready material."
                        : "Music licensing for content, partners, and commercial use."}
                </p>
              </article>
            ))}
          </div>
          <p className="en-inline-note">
            <strong>LICENSING NOTE · </strong>
            For music use in content, contact the studio by email. Commercial use
            is discussed separately; non-commercial use with clear credit is
            appreciated.
          </p>
        </section>
          ...
          <div className="en-section-intro">
            ...
            <p>
              {optionalAssets.length
                ? "Published studio assets are available through the cards below."
                : "Additional official event assets can be requested through the press contact."}
            </p>
          </div>
          <div className="en-service-grid">
            {optionalAssets.length ? (
              optionalAssets.map((item) => (
                <a
                  className="en-service-card"
                  href={item.url}
                  key={item.label}
                ...
                >
                  <span>{item.label}</span>
                  <h3>{item.title}</h3>
                  <p>Published file available through the official CMS.</p>
                  <ArrowUpRight size={16} />
                </a>
              ))
            ) : (
              <div className="en-empty">
                <span className="nf-page-eyebrow">ASSET STATUS</span>
                <strong>Available on request.</strong>
                <p>
                  Unavailable photo packs, riders, and contracts stay off the
                  public site until they are officially published.
                </p>
                <a
                  className="nf-button"
                  href={`mailto:${email}?subject=Press%20asset%20request`}
                >
                  <Mail size={14} /> REQUEST ASSETS
                </a>
              </div>
            )}
          </div>
        ...
        <section className="nf-section dark-panel">
          <div className="en-section-intro">
            ...
            <p>
              Official platform routes for editors, promoters, and collaborators.
            </p>
          </div>
          <div className="en-catalog">
            {catalog.slice(0, 3).map((release) => (
              <a
                className="en-release-card"
                href={release.href}
                target="_blank"
        ...
        <section className="nf-section en-contact-panel">
          <div className="en-two-column">
            <div>
              ...
              <p>
                Share the context for a performance, remix, collaboration, or
                licensing request. Availability and terms are confirmed after
                review.
              </p>
            </div>
            <div className="en-action-row">
              <Link
                className="nf-button"
                href="/en/inquire?type=booking&source=epk"
              >
                BOOKING INQUIRY <ArrowUpRight size={14} />
              </Link>
              <a
                className="nf-text-link nf-text-button"
                href={`mailto:${email}?subject=Press%20material%20request`}
              >
                PRESS CONTACT <Mail size={14} />
              </a>
            </div>
          </div>
        </section>
          ...
          <div className="nf-platform-grid">
            {editablePlatformLinks.slice(0, 5).map((link) => (
              ...
              >
                ...
                <span className="nf-platform-label">{link.label}</span>
                <ArrowUpRight size={14} />
              </a>
            ))}
          </div>
...
const inquiryServices = [
  [
    "01",
    "Booking",
    "For shows, curated programs, and event opportunities with a clear date and venue context.",
  ],
  [
    "02",
    "Remix requests",
    "Send the source, intended direction, and release or usage context for a considered response.",
  ],
  [
    "03",
    "Custom arrangements",
    "Production support for a defined brief, mood, reference, and delivery scope.",
  ],
  [
    "04",
    "Collaborations",
    "Bring a focused idea for a track, visual, or cross-disciplinary project.",
  ],
];
  ...
  return (
    <EnglishFrame>
      <main className="en-content">
        <section className="nf-section en-privacy-intro">
          <div className="en-two-column">
            <div className="en-copy-block">
              ...
              <p>
                This is the English reading of the public data note for the
                official Akbar Nawasunda website.
              </p>
            </div>
            <aside className="en-contact-panel">
              <ShieldCheck size={19} />
              <div>
                <span className="nf-page-eyebrow">DATA POSTURE</span>
                <h2>LIGHT BY DEFAULT.</h2>
                <p>
                  No advertising, no tracking cookies, and no sale of personal
                  data. The photo gallery only records anonymous aggregate
                  counts.
                </p>
              </div>
            </aside>
        ...
        <section className="nf-section">
          <div className="en-legal-layout">
            <nav className="en-legal-index" aria-label="Privacy Policy sections">
            ...
            <div>
              <article className="en-legal-block" id="short-version">
                ...
                <p className="en-legal-note">
                  No advertising, no tracking cookies, no selling data.
                </p>
                <p>
                  The site is designed to keep data collection limited to what is
                  needed for contact, site operation, and clearly described
                  services.
                </p>
              </article>
              <article className="en-legal-block" id="collection">
                ...
                <ul>
                  <li>
                    Contact details and project context when you send an inquiry.
                  </li>
                  <li>
                    Limited interface or authentication support stored by the
                    browser when required.
                  </li>
                  <li>
                    Anonymous aggregate counts for photo gallery access, without
                    IP, email, or user-agent storage.
                  </li>
                  <li>
                    JEDAG RUN leaderboard display names and scores when a player
                    chooses to submit a score.
                  </li>
                </ul>
              </article>
              <article className="en-legal-block" id="cookies">
                ...
                <p>
                  The site does not set advertising or tracking cookies. Limited
                  local storage may support interface preferences,
                  authentication, or an anonymous gallery visitor marker. The
                  gallery counter does not store IP, email, or user-agent. The
                  JEDAG RUN leaderboard stores only the public username and score
                  a player chooses to submit.
                </p>
                <p>
                  Spotify, YouTube, and SoundCloud players use a click-to-load
                  pattern. Their services receive requests only after you choose
                  to open or play them, and their own privacy policies then
                  apply.
                </p>
              </article>
              <article className="en-legal-block" id="services">
                ...
                <ul>
                  ...
                  <li>
                    JEDAG RUN leaderboard for player-submitted public names and
                    scores.
                  </li>
                  <li>
                    Spotify, YouTube, and SoundCloud for optional media players.
                  </li>
                  <li>
                    Apple Music and other platform services for official release
                    routes.
                  </li>
                </ul>
              </article>
              <article className="en-legal-block" id="rights">
                ...
                <p>
                  You may ask about access, correction, or deletion of personal
                  data held through an inquiry or site service. Requests should
                  be sent to the official contact address.
                </p>
                <a
                  className="nf-button"
                  href={`mailto:${bookingEmail}?subject=Privacy%20request`}
                >
                  <Mail size={14} /> CONTACT ABOUT YOUR DATA{" "}
                  <ArrowUpRight size={14} />
                </a>
              </article>
              <p className="en-inline-note">
                <CheckCircle2 size={13} /> Last reviewed: 25 Aug 2026 · Indonesia
              </p>
            </div>
          </div>
        </section>
...
export function EnglishReleaseDetail() {
  ...
  const release = catalog.find(
    (item) => releaseSlug(item.title) === params?.slug
  );
  ...
  const links = release
    ? [
        { label: release.platform, href: release.href },
    ...
    <EnglishFrame>
      <main className="en-content">
        <section
          ...
          style={
            {
              "--page-image": `url(${release?.image || officialBrand.socialPreview})`,
            } as CSSProperties
          }
        >
          <div>
        ...
        <section className="nf-section">
          <div className="en-two-column">
            <div className="en-release-card">
              <ResilientArtworkImage
                src={release?.image || officialBrand.socialPreview}
                backupSrc={officialBrand.socialPreview}
                alt={release ? `Artwork for ${release.title}` : "Release artwork loading"}
              />
              <div className="en-release-card-copy">
                <span>
                  <Disc3 size={12} /> {release?.format || "RELEASE"}
                </span>
              </div>
            </div>
            <div className="en-copy-block">
              ...
              <p className="en-inline-note">
                {release?.credits || "Official credits not published."}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="nf-section dark-panel">
          ...
          <div className="en-service-grid release-platform-links">
            {links.map((link) => (
              <a
                className="en-service-card"
                key={link.label}
            ...
            <Link className="en-service-card" href="/en/licensing">
              <span>USAGE</span>
              <h3>{rightsAction(release?.format)}</h3>
              <ArrowUpRight size={16} />
            </Link>
          </div>
        </section>
