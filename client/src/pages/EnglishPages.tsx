...
export function EnglishAbout() {
  ...
  return <EnglishFrame><main className="en-content">
    <section className="nf-page-hero en-hero" style={{ "--page-image": `url(${profile?.portraitImage || officialBrand.portrait})` } as React.CSSProperties}><div><p className="nf-page-eyebrow">ABOUT / ARTIST PROFILE</p><h1>AKBAR<br /><em>NAWASUNDA.</em></h1><p>{shortBio}</p></div><div className="nf-hero-note"><span>BASED IN</span>{locationUrl ? <a className="nf-location-link" href={locationUrl} target="_blank" rel="noreferrer"><strong>{location}</strong><ArrowUpRight size={13} /></a> : <strong>{location}</strong>}<Link className="nf-text-button" href="/en/music">VIEW MUSIC <ArrowUpRight size={15} /></Link></div></section>
