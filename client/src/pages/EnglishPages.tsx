...
import { publicJourney, publicPhotoStories, publicPlatformLinks, publicPortraitStudies, publicUpcomingEvents, usePublicArtistContent } from "@/content/publicContent";
...
const rightsLabel = (format?: string) => /bootleg/i.test(format || "") ? "UNOFFICIAL EDIT · CLEARANCE REQUIRED" : /remix|rmx/i.test(format || "") ? "REMIX · CLEARANCE REQUIRED" : "CATALOG ENTRY";
const rightsAction = (format?: string) => /bootleg/i.test(format || "") ? "CLEARANCE / ASK FIRST" : "LICENSING / ASK FIRST";
...
export function EnglishHome() {
  ...
  const featuredEvent = publicEvents.find(event => event.isFeatured) || publicEvents[0];

  return <EnglishFrame><main id="top" className="en-content en-home-parity">
    <section className="an-hero en-parity-hero">
      <div className="home-hero-portrait"><FragmentRevealImage src={officialBrand.portrait} alt="Portrait of Akbar Nawasunda" width={800} height={1000} fetchPriority="high" decoding="async" /></div>
    ...
    <section ref={platformSectionRef} className="home-signal-deck reveal-target" aria-labelledby="en-signal-deck-title"><SectionIndex number="01" label="LISTENING LINKS" /><div className="home-signal-copy"><p className="eyebrow"><span /> OFFICIAL PLATFORMS</p><h2 id="en-signal-deck-title">LISTEN<br />ANYWHERE.</h2><Link className="home-deck-cta" href="/en/music">VIEW MUSIC <ArrowUpRight size={15} /></Link></div><div className="home-platform-rack">{editablePlatformLinks.map((platform, index) => <a className={`home-platform-card platform-${platform.label.toLowerCase().replace(/\s+/g, "-")}`} key={platform.label} href={platform.href} target="_blank" rel="noreferrer" aria-label={`Open Akbar Nawasunda on ${platform.label}`}><span className="home-platform-number">{formatPublicIndex(index)}</span><span className="home-platform-icon-shell"><PlatformIcon label={platform.label} /></span><span className="home-platform-copy"><strong>{platform.label}</strong></span><ArrowUpRight className="home-platform-arrow" size={15} /></a>)}</div><PlatformMarquee links={editablePlatformLinks} /></section>

    <section ref={currentSectionRef} className="section section-current reveal-target" id="music"><SectionIndex number="02" label="FEATURED RELEASE" /><div className="section-heading"><p className="eyebrow">LATEST RELEASE</p><h2>NOW<br />PLAYING.</h2></div><div className="feature-release"><div className="release-cover"><ResilientArtworkImage src={activeRelease.image} backupSrc={officialBrand.socialPreview} alt={`Artwork for ${activeRelease.title}`} /><span className="cover-orbit" /></div><div className="release-detail"><p className="mono-label">{activeRelease.format} · {activeRelease.year}</p><h3>{activeRelease.title}</h3><p>Open the official release route to listen to this verified catalog entry.</p><div className="release-detail-actions"><a className="text-link" href={activeRelease.href} target="_blank" rel="noreferrer">OPEN RELEASE <ArrowUpRight size={16} /></a><span>{activeRelease.platform}</span></div></div></div></section>

    <div className="an-scroll-scene an-scroll-scene-artist-story" data-scroll-scene="artist-story"><ArtistEditorialSections journey={journey} photoStories={photoStories} locale="en" /></div>
    <HorizontalReleaseSequence locale="en" label="Akbar Nawasunda homepage release catalog" itemCount={catalog.length} className="release-section"><SectionIndex number="05" label="CATALOG" /><div className="section-inline"><div><p className="eyebrow">DISCOGRAPHY / SCROLL-THROUGH</p><h2>ALL<br />RELEASES.</h2></div><a className="text-link" href={editablePlatformLinks.find(link => link.label === "Spotify")?.href || "https://open.spotify.com/intl-id/artist/7KOQuIQLuxyklLox0RDMMw"} target="_blank" rel="noreferrer">SPOTIFY <PlatformIcon label="Spotify" /> <ArrowUpRight size={16} /></a></div><div className="nf-horizontal-release-viewport" data-horizontal-viewport="true"><div className="release-grid" data-horizontal-track="true">{catalog.map((release, index) => <a className="release-card" key={release.title} href={release.href} target="_blank" rel="noreferrer">{release.image && <div className="release-card-art"><ResilientArtworkImage src={release.image} backupSrc={officialBrand.socialPreview} alt={`Artwork for ${release.title}`} /></div>}<span className="release-number">{formatPublicIndex(index)}</span><PlatformIcon label={release.platform} /><p>{release.format} · {release.year}</p><h3>{release.title}</h3><span className="release-platform-line">{release.platform} <ArrowUpRight size={14} /></span></a>)}</div></div></HorizontalReleaseSequence>

    <section ref={visualSectionRef} className="section visual-section reveal-target"><SectionIndex number="06" label="VISUALS" /><div className="section-heading"><p className="eyebrow">VISUAL</p><h2>OFFICIAL<br />VIDEO.</h2></div><div className="video-grid">{activeVideos.map((video, index) => <a key={video.title} className={`video-card video-${index + 1}`} href={video.href} target="_blank" rel="noreferrer"><ResilientArtworkImage src={video.image} backupSrc={officialBrand.socialPreview} alt={`${video.title} — official visual artwork`} /><div className="video-overlay" /><div className="video-content"><span>{video.label}</span><h3>{video.title}</h3><div className="round-play"><Play size={17} fill="currentColor" /></div></div></a>)}</div></section>

    <section ref={liveSectionRef} className="section live-section reveal-target"><SectionIndex number="07" label="LIVE" /><div className="live-backdrop" style={{ backgroundImage: "url(/assets/akbar-night-frequency-hero-optimized.webp)" }} /><div className="live-copy"><p className="eyebrow">LIVE</p><h2>LIVE<br />DATES.</h2><p>{cms.data?.live?.message || "Dates will appear here after they are officially announced."}</p><Link className="button-primary" href={featuredEvent?.ticketUrl || featuredEvent?.rsvpUrl || "/en/inquire"} target={featuredEvent?.ticketUrl || featuredEvent?.rsvpUrl ? "_blank" : undefined} rel={featuredEvent?.ticketUrl || featuredEvent?.rsvpUrl ? "noreferrer" : undefined}><Ticket size={16} /><span>{featuredEvent?.ticketUrl ? "GET TICKETS" : featuredEvent?.rsvpUrl ? "RSVP SHOW" : "ASK ABOUT BOOKING"}</span></Link></div><div className="live-status"><span>{featuredEvent ? "NEXT CONFIRMED SHOW" : "DATES"}</span><strong>{featuredEvent?.title || <>NO PUBLIC<br />DATE</>}</strong><small>{featuredEvent ? formatEnglishDate(featuredEvent.date) : "TO BE ANNOUNCED"}</small></div>{publicEvents.length ? <div className="home-live-events" aria-label="Confirmed live dates">{publicEvents.slice(0, 3).map(event => { const eventHref = event.ticketUrl || event.rsvpUrl || "/en/live"; const locationLabel = [event.venue, event.city, event.country].filter(Boolean).join(", ") || "Venue details to follow"; return <article className="home-live-event" key={event._id}><span>{formatEnglishDate(event.date, event.time)}</span><strong>{event.title}</strong><small>{event.mapsUrl ? <a className="en-event-location-link" href={event.mapsUrl} target="_blank" rel="noreferrer">{locationLabel} <ArrowUpRight size={12} /></a> : locationLabel}</small><a className="home-live-event-source" href={eventHref} target={eventHref.startsWith("http") ? "_blank" : undefined} rel={eventHref.startsWith("http") ? "noreferrer" : undefined} aria-label={`${event.title} — open official source`}>{event.ticketUrl ? "TICKETS" : event.rsvpUrl ? "RSVP" : "VIEW DATES"} <ArrowUpRight size={14} /></a></article>; })}</div> : null}</section>

    <section ref={signalSectionRef} className="signal-section reveal-target" id="signal"><SectionIndex number="08" label="NEWS" /><div><p className="eyebrow"><Sparkles size={14} /> LATEST UPDATES</p><h2>NEW MUSIC<br />AND NEWS.</h2><p>Official release, visual, and live updates from Akbar Nawasunda's channels.</p></div><div className="en-signal-action"><strong>FOLLOW THE OFFICIAL LINKS.</strong><p>Follow the official platforms for new music and future announcements.</p><Link className="button-primary" href="/en/inquire">CONTACT AKBAR <ArrowUpRight size={15} /></Link></div></section>
  </main></EnglishFrame>;
}
...
export function EnglishMusic() {
  ...
  return <EnglishFrame><main className="en-content"><section className="nf-page-hero en-hero" style={{ "--page-image": `url(${featured.image || officialBrand.socialPreview})` } as React.CSSProperties}><div><p className="nf-page-eyebrow">MUSIC / OFFICIAL CATALOG</p><h1>MUSIC<br /><em>BY AKBAR.</em></h1><p>Original releases and remixes by Akbar Nawasunda.</p></div><div className="nf-hero-note"><span>{cms.isLoading ? "LOADING CATALOG" : "FEATURED RELEASE"}</span><strong>{featured.title}</strong><a className="nf-text-button" href={featured.href} target="_blank" rel="noreferrer">LISTEN <ArrowUpRight size={15} /></a></div></section>
    ...
    <HorizontalReleaseSequence locale="en" label="Akbar Nawasunda release catalog" itemCount={catalog.length}><div className="en-section-intro"><p className="nf-page-eyebrow">FULL CATALOG / SCROLL-THROUGH</p><h2>EVERY<br /><em>RELEASE.</em></h2><p>Scroll down to move across the complete catalog. Each title keeps its official release route.</p></div><div className="nf-horizontal-release-viewport" data-horizontal-viewport="true"><div className="en-catalog" data-horizontal-track="true">{catalog.map((item, index) => <Link key={`${item.title}-${index}`} className="en-release-card" href={`/en/music/${releaseSlug(item.title)}`}><ResilientArtworkImage src={item.image} backupSrc={officialBrand.socialPreview} alt={`Artwork for ${item.title}`} /><div className="en-release-card-copy"><span>{formatPublicIndex(index)} / {item.format} · {item.year}</span><h3>{item.title}</h3><p>{item.platform}</p></div></Link>)}</div></div></HorizontalReleaseSequence></main></EnglishFrame>;
}
...
export function EnglishVisuals() {
  ...
  return <EnglishFrame><main className="en-content"><section className="nf-page-hero en-hero" style={{ "--page-image": `url(${portraitContent[0]?.imageUrl || officialBrand.portrait})` } as React.CSSProperties}><div><p className="nf-page-eyebrow">VISUALS / OFFICIAL CHANNEL</p><h1>VIDEO<br /><em>ARCHIVE.</em></h1><p>Visual releases, DJ edits, and official uploads from the Akbar Nawasunda channel.</p></div><div className="nf-hero-note"><span>{cms.isLoading ? "LOADING VISUALS" : "OFFICIAL CHANNEL"}</span><strong>AKBAR NAWASUNDA</strong><a className="nf-text-button" href="https://www.youtube.com/@akbarnawasunda" target="_blank" rel="noreferrer">OPEN YOUTUBE <ArrowUpRight size={15} /></a></div></section><section className="nf-section"><div className="en-section-intro"><p className="nf-page-eyebrow">SELECTED VISUALS</p><h2>WATCH<br /><em>THE VIDEOS.</em></h2><p>Preview thumbnails first. Embedded players load only after you choose to play.</p></div><div className="en-visual-grid">{visualItems.map(item => item.youtubeId ? <div className="en-visual-card" key={item.title}><OfficialMediaFrame title={item.title} provider="YouTube" sourceUrl={item.href} embedUrl={`https://www.youtube-nocookie.com/embed/${item.youtubeId}`} artwork={item.image} backupArtwork={officialBrand.socialPreview} description="Official video channel." /></div> : <a className="en-service-card" href={item.href} target="_blank" rel="noreferrer"><span>{item.label}</span><h3>{item.title}</h3><p>Open the official source to view this visual.</p><ArrowUpRight size={17} /></a>)}</div></section>      <VisualPortraitStudies english studies={portraitContent} /> <ArtistEditorialSections photoStories={publicPhotoStories(cms.data)} locale="en" /> <section className="nf-section dark-panel"><div className="en-contact-panel"><div><p className="nf-page-eyebrow">DIRECTOR / BOOKER / CURATOR</p><h2>NEED THE<br /><em>RIGHT CUT?</em></h2><p>For visual collaboration, licensing, or a release-related brief, send the context directly.</p></div><a className="nf-button" href={`mailto:${bookingEmail}?subject=Visual%20inquiry`}><Mail size={15} /> EMAIL THE STUDIO <ArrowUpRight size={15} /></a></div></section></main></EnglishFrame>;
}
...
export function EnglishUniverse() {
  ...
  const journey = publicJourney(cms.data);
  const photoStories = publicPhotoStories(cms.data);
  return <EnglishFrame><main className="en-content"><section className="nf-page-hero en-hero"><div><p className="nf-page-eyebrow">AN ARCHIVE / ORIGIN</p><h1>THE<br /><em>UNIVERSE.</em></h1><p>A concise route through the sound, identity, and catalog behind Akbar Nawasunda.</p></div><div className="nf-hero-note"><span>ORIGIN</span><strong>2020 / BANDUNG BARAT</strong><a className="nf-text-button" href="#origin">READ THE STORY <ArrowUpRight size={15} /></a></div></section><section className="nf-section" id="origin"><div className="en-two-column"><div className="en-aside"><p className="nf-page-eyebrow">THE BEGINNING</p><strong>From DJ Akbar Remix to Akbar Nawasunda.</strong><span>Independent producer · Bandung Barat · Indonesia</span></div><div className="en-copy-block"><p className="nf-page-eyebrow">THE THROUGH-LINE</p><h2>REBUILD THE<br /><em>ENERGY.</em></h2><p>{journey.introEn || "Akbar Nawasunda began creating independently in 2020 as DJ Akbar Remix, experimenting with Breakbeat, Jedag Jedug, and Jungle Dutch through a Bandung-rooted remix language. The current name carries that energy into original releases that connect pop melody, electronic bass, and remix instinct."}</p><div className="en-stat-row"><div className="en-stat"><strong>2020</strong><span>CREATIVE ORIGIN</span></div><div className="en-stat"><strong>{verifiedArtistProfile.location.split(",")[0]}</strong><span>HOME BASE</span></div></div><div className="en-genre-row">{verifiedArtistProfile.genres.map(genre => <span key={genre}>{genre}</span>)}</div><div className="an-archive-timeline">{journey.milestones.map((milestone, index) => <article key={`${milestone.year}-${milestone.title}-${index}`}><span>{milestone.year}</span><div><h3>{milestone.titleEn || milestone.title}</h3><p>{milestone.bodyEn || milestone.body}</p></div></article>)}</div></div></div></section><ArtistEditorialSections photoStories={photoStories} locale="en" /><section className="nf-section en-archive-art-feature"><div className="en-archive-art-frame"><img src={officialBrand.archivePortrait} alt="Editorial Akbar Nawasunda artwork about a future city" loading="lazy" decoding="async" width={800} height={1000} /></div><div className="en-archive-art-copy"><p className="nf-page-eyebrow">VISUAL LANGUAGE / 001</p><h2>THE FUTURE<br />IS HERE.</h2><p>One visual direction from the Akbar Nawasunda world: industrial, high-contrast, and close to electronic bass energy.</p><p className="en-archive-art-note">ARCHIVE VISUAL · NOT AN AUDIO RELEASE</p><Link className="nf-button" href="/en/visuals">VIEW VISUALS <ArrowUpRight size={15} /></Link></div></section><HorizontalReleaseSequence locale="en" label="Akbar Nawasunda Archive release catalog" itemCount={Math.min(6, catalog.length)} className="dark-panel"><div className="en-section-intro"><p className="nf-page-eyebrow">RELEASED WORK / SCROLL-THROUGH</p><h2>THE<br /><em>CATALOG.</em></h2><p>Open each title through the official platform route. Scroll down to move across the archive.</p></div><div className="nf-horizontal-release-viewport" data-horizontal-viewport="true"><div className="en-catalog" data-horizontal-track="true">{catalog.slice(0, 6).map((item, index) => <Link key={`${item.title}-${index}`} className="en-release-card" href={`/en/music/${releaseSlug(item.title)}`}><ResilientArtworkImage src={item.image} backupSrc={officialBrand.socialPreview} alt={`Artwork for ${item.title}`} /><div className="en-release-card-copy"><span>{item.year} / {item.platform}</span><h3>{item.title}</h3></div></Link>)}</div></div></HorizontalReleaseSequence></main></EnglishFrame>;
}
...
export function EnglishAbout() {
  ...
  const journey = publicJourney(cms.data);
  const longBio = journey.introEn || englishLongBio;
  ...
  return <EnglishFrame><main className="en-content">
    <section className="nf-page-hero en-hero en-about-hero" style={{ "--page-image": `url(${profile?.portraitImage || officialBrand.portrait})` } as React.CSSProperties}><div><p className="nf-page-eyebrow">ABOUT / ARTIST PROFILE</p><h1>AKBAR<br /><em>NAWASUNDA.</em></h1><p>{shortBio}</p></div><div className="nf-hero-note"><span>BASED IN</span>{locationUrl ? <a className="nf-location-link" href={locationUrl} target="_blank" rel="noreferrer"><strong>{location}</strong><ArrowUpRight size={13} /></a> : <strong>{location}</strong>}<Link className="nf-text-button" href="/en/music">VIEW MUSIC <ArrowUpRight size={15} /></Link></div></section>
    <section className="nf-section an-profile-section"><div className="an-profile-aside"><p className="nf-page-eyebrow">ARTIST PROFILE</p><MapPin size={18} />{locationUrl ? <a className="an-profile-location-link" href={locationUrl} target="_blank" rel="noreferrer"><span>{location}</span><ArrowUpRight size={13} /></a> : <span>{location}</span>}<span>ALSO KNOWN AS {verifiedArtistProfile.aliases.join(" / ")}</span></div><div className="an-profile-copy"><h2>MUSIC<br />JOURNEY.</h2><p>{longBio}</p>{profile?.artistStatement ? <blockquote>“{profile.artistStatement}”</blockquote> : <p className="an-profile-note">Original work is released as Akbar Nawasunda; the remix catalog is also known through DJ Akbar Remix.</p>}<div className="an-genre-row">{genres.map(genre => <span key={genre}><Radio size={12} /> {genre}</span>)}</div></div></section>
...
export function EnglishEpk() {
  ...
  return <EnglishFrame><main className="en-content">
    ...
    <HorizontalReleaseSequence locale="en" label="Selected press releases by Akbar Nawasunda" itemCount={Math.min(3, catalog.length)} className="dark-panel"><div className="en-section-intro"><p className="nf-page-eyebrow">SELECTED RELEASES / SCROLL-THROUGH</p><h2>RELEASED<br /><em>WORK.</em></h2><p>Official platform routes for editors, promoters, and collaborators. Scroll down to move across the selected releases.</p></div><div className="nf-horizontal-release-viewport" data-horizontal-viewport="true"><div className="en-catalog" data-horizontal-track="true">{catalog.slice(0, 3).map((release, index) => <a className="en-release-card" href={release.href} target="_blank" rel="noreferrer" key={release.title}><ResilientArtworkImage src={release.image} backupSrc={officialBrand.socialPreview} alt={`Artwork for ${release.title}`} /><div className="en-release-card-copy"><span>{release.platform} · {release.year}</span><h3>{release.title}</h3><p>{release.format}</p></div></a>)}</div></div></HorizontalReleaseSequence>
...
export function EnglishReleaseDetail() {
  ...
  return <EnglishFrame><main className="en-content"><section className="nf-page-hero en-hero" style={{ "--page-image": `url(${release?.image || officialBrand.socialPreview})` } as React.CSSProperties}><div><Link className="en-back-link" href="/en/music"><ArrowLeft size={14} /> BACK TO MUSIC</Link><p className="nf-page-eyebrow">RELEASE / {release?.format || "LOADING"}</p><h1>{release?.title || "LOADING RELEASE…"}</h1><p>{release ? `${release.format} · ${release.year} · ${release.platform}` : "Loading the official catalog."}</p></div><div className="nf-hero-note"><span>{rightsLabel(release?.format)}</span><strong>{release?.year || "—"}</strong>{release && <a className="nf-text-button" href={release.href} target="_blank" rel="noreferrer">LISTEN <ArrowUpRight size={15} /></a>}</div></section><section className="nf-section"><div className="en-two-column"><div className="en-release-card"><ResilientArtworkImage src={release?.image || officialBrand.socialPreview} backupSrc={officialBrand.socialPreview} alt={release ? `Artwork for ${release.title}` : "Release artwork loading"} /><div className="en-release-card-copy"><span><Disc3 size={13} /> {release?.format || "RELEASE"}</span></div></div><div className="en-copy-block"><p className="nf-page-eyebrow">ABOUT THE RELEASE</p><h2>{release?.title || "RELEASE"}</h2><p>Release details will appear when published. Use the listed source link to listen.</p><p className="en-inline-note">{release?.credits || "Official credits not published."}</p></div></div></section><section className="nf-section dark-panel"><div className="en-section-intro"><p className="nf-page-eyebrow">LISTEN ON</p><h2>OFFICIAL<br /><em>PLATFORMS.</em></h2></div><div className="en-service-grid release-platform-links">{links.map(link => <a className="en-service-card" key={link.label} href={link.href} target="_blank" rel="noreferrer"><span>{link.label}</span><h3>OPEN PLATFORM</h3><ArrowUpRight size={17} /></a>)}<Link className="en-service-card" href="/en/licensing"><span>USAGE</span><h3>{rightsAction(release?.format)}</h3><ArrowUpRight size={17} /></Link></div></section></main></EnglishFrame>;
}
