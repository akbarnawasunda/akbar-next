...
export default function Universe() {
  ...
  return <div className="nf-page an-archive-page"><NightHeader active="/universe" /><main>
    ...
    <HorizontalReleaseSequence locale="id" label="Katalog rilisan pilihan Archive Akbar Nawasunda" className="dark-panel"><div className="nf-section-title"><div><p className="nf-page-eyebrow">RILISAN PILIHAN / SCROLL-THROUGH</p><h2>ARTWORK<br/>RILISAN.</h2></div><p>Buka di platform resmi. Scroll ke bawah untuk melewati seluruh pilihan rilisan.</p></div><div className="nf-horizontal-release-viewport" data-horizontal-viewport="true"><div className="an-archive-release-grid" data-horizontal-track="true">{catalog.slice(0, 6).map(release => <a className="an-archive-release" href={release.href} target="_blank" rel="noreferrer" key={release.title}><ResilientArtworkImage src={release.image} backupSrc={officialBrand.socialPreview} alt={`Artwork ${release.title}`} /><div><span>{release.year} · {release.platform}</span><h3>{release.title}</h3><p>BUKA RILISAN <ArrowUpRight size={13}/></p></div></a>)}</div></div></HorizontalReleaseSequence>
