...
export default function PressKit() {
  ...
  return <div className="nf-page"><NightHeader /><main><section className="nf-epk-hero"><p className="nf-page-eyebrow">AKBAR NAWASUNDA / PRESS & BOOKING</p><ScrambleText as="h1" interactive duration={2400} text={"THE PRESS\nSIGNAL."} /><p>{press?.intro || "Materi ringkas untuk promotor, media, playlist editor, dan calon kolaborator. Akbar Nawasunda adalah produser dan remixer elektronik dari Bandung Barat, Indonesia."}</p><div className="nf-press-actions"><a className="nf-button" href={mail(pressEmail, "Press asset request")}><Mail size={16}/> REQUEST PRESS ASSETS</a><a className="nf-text-button" href="/inquire?type=booking&source=epk">BOOKING INQUIRY <ArrowUpRight size={15}/></a></div></section><section className="nf-section"><div className="an-press-intro"><p className="nf-page-eyebrow">PRESS MATERIALS</p><ScrambleText as="h2" interactive duration={2400} text={"READY WHEN\nTHE SIGNAL IS."} /><p>Download tersedia langsung saat materi resmi sudah diterbitkan melalui Studio. Sebelum itu, setiap request dikirim ke jalur resmi agar format, resolusi, dan hak penggunaan dapat disiapkan dengan benar.</p></div><div className="an-press-grid">{packs.map(pack => <article className="an-press-pack" key={pack.title}><span>{pack.label}</span><div><h3>{pack.title}</h3><p>{pack.copy}</p></div>{pack.href ? <a href={pack.href} target="_blank" rel="noreferrer"><ArrowDownToLine size={14}/> DOWNLOAD <ArrowUpRight size={13}/></a> : <a href={mail(pressEmail, pack.request)}><PackageOpen size={14}/> REQUEST PACK <ArrowUpRight size={13}/></a>}</article>)}</div></section><section className="nf-section dark-panel"><div className="an-booking-grid"><div><p className="nf-page-eyebrow">BOOKING / COLLABORATION</p><ScrambleText as="h2" interactive duration={2400} text={"BUILD THE\nNEXT ROOM."} /><p>Jalur resmi tersedia untuk {verifiedArtistProfile.services.join(", ").toLowerCase()}, dan kebutuhan press. {verifiedArtistProfile.licensing}</p></div><div className="an-booking-actions"><a href="/inquire?type=booking&source=epk">BOOK A PERFORMANCE <ArrowUpRight size={16}/></a><a href="/inquire?type=remix&source=epk">REMIX / COLLABORATE <ArrowUpRight size={16}/></a><a href="/licensing">LICENSING & USAGE <FileText size={16}/></a></div></div></section></main><NightFooter /></div>;
}
