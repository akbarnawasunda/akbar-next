...
export default function Music() {
  ...
  return <div className="nf-page"><NightHeader active="/music" /><main>
    ...
    <section className="nf-section dark-panel"><div className="nf-section-title"><div><p className="nf-page-eyebrow">KATALOG</p><h2>SEMUA<br/>RILISAN.</h2></div><p>{cmsReleases.length ? "Rilisan yang sedang dikelola dan arsip resmi." : "Kumpulan rilisan Akbar Nawasunda."}</p></div><div className="nf-catalog">{catalog.map((release, index) => <Link key={`${release.title}-${index}`} className="nf-catalog-card" href={`/music/${releaseSlug(release.title)}`}><ResilientArtworkImage className="nf-catalog-art" src={release.image} backupSrc={officialBrand.socialPreview} alt={`Artwork ${release.title}`} /><span className="index">{formatPublicIndex(index)}</span><PlatformIcon label={release.platform} /><p>{release.format} · {release.year}</p><h3>{release.title}</h3><b>{release.platform}<ArrowUpRight size={14} /></b></Link>)}</div></section>
