...
export default function Visuals() {
  ...
  return <div className="nf-page"><NightHeader active="/visuals" /><main>
    ...
    <section className="nf-section"><div className="nf-section-title"><div><p className="nf-page-eyebrow">VIDEO PILIHAN</p><h2>DENGAR<br/>DAN TONTON.</h2></div><p>Tekan play untuk memuat player, atau buka YouTube langsung.</p></div><div className="nf-video-embed-grid">{players.map(video => <OfficialMediaFrame key={video.id} title={video.title} provider="YouTube" sourceUrl={`https://youtu.be/${video.id}`} embedUrl={`https://www.youtube-nocookie.com/embed/${video.id}`} artwork={thumbnailFor(video.id)} backupArtwork={officialBrand.socialPreview} description="Video dari channel resmi Akbar Nawasunda." />)}</div></section>
